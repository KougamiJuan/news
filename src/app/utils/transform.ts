...
export function xmlToJson(response: string): { info: {}; items: any[] } {
  ...
  const info: any = {};
  (xmlData.querySelectorAll('channel>*:not(item)') || []).forEach(
    (chanel: any) => {
      const infoData: any = {};
      let found = false;
      ...
      if (found) {
        info[chanel.tagName] = infoData;
      } else {
        info[chanel.tagName] = chanel.textContent;
      }
    }
  );
  ...
  (xmlData.querySelectorAll('channel>item') || []).forEach((item: any) => {
    ...
    item.childNodes.forEach((e: any) => {
      ...
      if (found) {
        itemData[e.tagName] = attr;
      } else {
        itemData[e.tagName] = e.textContent;
      }
    });
    items.push(itemData);
  });
