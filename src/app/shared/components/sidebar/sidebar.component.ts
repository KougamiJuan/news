...
import { FeedRssService } from 'src/app/core/services/feed-rss.service';
...
export class SidebarComponent implements OnInit {
    ...
    this.feeds.forEach((feed) => {
      feed.categories.forEach((category: { completed: any; name: any }) => {
        if (!category.completed) {
          const index = newSources.findIndex(
            (source) =>
              (source.info.category || source.info.description) ===
              category.name
          );
  ...
  someComplete(feed: any): boolean {
    ...
    return (
      feed.categories.filter((check: any) => check.completed).length > 0 &&
      !feed.allComplete
    );
  }
